...
class TestDeploymentFrequencyTrend:
    ...
    def test_deployment_frequency_trend_weekly_aggregation(self):
        ...
        # Week 1: 2 releases
        releases.append({'tag_name': 'v1.0.0', 'environment': 'production',
                        'published_at': base_date, 'repo': 'test/repo'})
        releases.append({'tag_name': 'v1.0.1', 'environment': 'production',
                        'published_at': base_date + timedelta(days=2), 'repo': 'test/repo'})

        # Week 2: 3 releases
        releases.append({'tag_name': 'v1.0.2', 'environment': 'production',
                        'published_at': base_date + timedelta(days=7), 'repo': 'test/repo'})
        releases.append({'tag_name': 'v1.0.3', 'environment': 'production',
                        'published_at': base_date + timedelta(days=9), 'repo': 'test/repo'})
        releases.append({'tag_name': 'v1.0.4', 'environment': 'production',
                        'published_at': base_date + timedelta(days=12), 'repo': 'test/repo'})

        # Week 3: 1 release
        releases.append({'tag_name': 'v1.0.5', 'environment': 'production',
                        'published_at': base_date + timedelta(days=14), 'repo': 'test/repo'})
    ...
    def test_deployment_frequency_trend_filters_staging(self):
        ...
        releases = [
            {'tag_name': 'v1.0.0', 'environment': 'production',
             'published_at': datetime(2025, 1, 1, tzinfo=timezone.utc), 'repo': 'test/repo'},
            {'tag_name': 'v1.0.1-rc', 'environment': 'staging',
             'published_at': datetime(2025, 1, 2, tzinfo=timezone.utc), 'repo': 'test/repo'},
            {'tag_name': 'v1.0.2', 'environment': 'production',
             'published_at': datetime(2025, 1, 3, tzinfo=timezone.utc), 'repo': 'test/repo'},
        ]
...
class TestLeadTimeTrend:
    ...
    def test_lead_time_trend_weekly_median(self):
        ...
        prs = [
            {'pr_number': 1, 'merged': True,
             'merged_at': base_date, 'author': 'user1', 'title': 'PR 1'},
            {'pr_number': 2, 'merged': True,
             'merged_at': base_date + timedelta(hours=1), 'author': 'user1', 'title': 'PR 2'},
        ]

        # Week 2: PRs with 5h and 7h lead times (median: 6h)
        prs.extend([
            {'pr_number': 3, 'merged': True,
             'merged_at': base_date + timedelta(days=7), 'author': 'user1', 'title': 'PR 3'},
            {'pr_number': 4, 'merged': True,
             'merged_at': base_date + timedelta(days=7, hours=1), 'author': 'user1', 'title': 'PR 4'},
        ])

        # Releases to map PRs to
        releases = [
            {'tag_name': 'v1.0.0', 'environment': 'production',
             'published_at': base_date + timedelta(hours=2), 'repo': 'test/repo'},  # 1h and 3h after PRs
            {'tag_name': 'v1.0.1', 'environment': 'production',
             'published_at': base_date + timedelta(days=7, hours=7), 'repo': 'test/repo'},  # 5h and 7h after PRs
        ]
    ...
    def test_lead_time_trend_with_jira_mapping(self):
        ...
        prs = [
            {'pr_number': 1, 'merged': True,
             'merged_at': base_date, 'author': 'user1', 'title': '[PROJ-123] Feature'},
        ]

        releases = [
            {'tag_name': 'Live - 2/Jan/2025', 'environment': 'production',
             'published_at': base_date + timedelta(hours=12), 'repo': 'test/repo'},
        ]

        # Issue to version mapping
        issue_to_version_map = {
            'PROJ-123': 'Live - 2/Jan/2025'
        }
...
class TestChangeFailureRateTrend:
    ...
    def test_cfr_trend_weekly_percentage(self):
        ...
        releases.extend([
            {'tag_name': 'v1.0.2', 'environment': 'production',
             'published_at': base_date + timedelta(days=7), 'repo': 'test/repo'},
            {'tag_name': 'v1.0.3', 'environment': 'production',
             'published_at': base_date + timedelta(days=9), 'repo': 'test/repo'},
            {'tag_name': 'v1.0.4', 'environment': 'production',
             'published_at': base_date + timedelta(days=12), 'repo': 'test/repo'},
        ])

        # Incidents correlated to deployments
        incidents = [
            {'key': 'INC-1', 'created': base_date + timedelta(hours=2),
             'resolved': base_date + timedelta(hours=5),
             'related_deployment': 'v1.0.0'},  # Week 1 failure
            {'key': 'INC-2', 'created': base_date + timedelta(days=7, hours=2),
             'resolved': base_date + timedelta(days=7, hours=6),
             'related_deployment': 'v1.0.2'},  # Week 2 failure
            {'key': 'INC-3', 'created': base_date + timedelta(days=9, hours=1),
             'resolved': base_date + timedelta(days=9, hours=3),
             'related_deployment': 'v1.0.3'},  # Week 2 failure
        ]
