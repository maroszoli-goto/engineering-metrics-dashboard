    ...
    def calculate_performance_score(metrics, all_metrics_list, team_size=None, weights=None):
        ...
        if cycle_time_values and metrics.get("cycle_time", 0) > 0:
            cycle_time_score = MetricsCalculator.normalize(
                metrics.get("cycle_time", 0), min(cycle_time_values), max(cycle_time_values)
            )
            score += (100 - cycle_time_score) * weights["cycle_time"]

        if jira_values and max(jira_values) > 0:
            jira_score = MetricsCalculator.normalize(
                metrics.get("jira_completed", 0), min(jira_values), max(jira_values)
            )
        ...
        if merge_rate_values and max(merge_rate_values) > 0:
            merge_rate_score = MetricsCalculator.normalize(
                metrics.get("merge_rate", 0), min(merge_rate_values), max(merge_rate_values)
            )
        ...
        if "deployment_frequency" in weights and weights["deployment_frequency"] > 0:
            ...
            if (
                deployment_freq_values
                and max(deployment_freq_values) > 0
                and metrics.get("deployment_frequency") is not None
            ):
                deployment_freq_score = MetricsCalculator.normalize(
                    metrics.get("deployment_frequency", 0), min(deployment_freq_values), max(deployment_freq_values)
                )
        ...
        if "lead_time" in weights and weights["lead_time"] > 0:
            ...
            if lead_time_values and metrics.get("lead_time") is not None and metrics.get("lead_time", 0) > 0:
                lead_time_score = MetricsCalculator.normalize(
                    metrics.get("lead_time", 0), min(lead_time_values), max(lead_time_values)
                )
        ...
        if "change_failure_rate" in weights and weights["change_failure_rate"] > 0:
            ...
            if cfr_values and max(cfr_values) > 0 and metrics.get("change_failure_rate") is not None:
                cfr_score = MetricsCalculator.normalize(
                    metrics.get("change_failure_rate", 0), min(cfr_values), max(cfr_values)
                )
        ...
        if "mttr" in weights and weights["mttr"] > 0:
            ...
            if mttr_values and metrics.get("mttr") is not None and metrics.get("mttr", 0) > 0:
                mttr_score = MetricsCalculator.normalize(metrics.get("mttr", 0), min(mttr_values), max(mttr_values))
